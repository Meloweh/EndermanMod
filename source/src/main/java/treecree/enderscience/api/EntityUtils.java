...
import treecree.enderscience.api.models.IMorphProvider;
...
public class EntityUtils
{
    ...
    public static NBTTagCompound stripEntityNBT(NBTTagCompound tag)
    {
        ...
        if (tag.hasKey("Attributes"))
        {
            ...
            {
            	NBTTagCompound deltaTag = attributes.getCompoundTagAt(i);
            	
            	final ResourceLocation resource = new ResourceLocation(deltaTag.getString(ResourceReference.RESOURCE_DOMAIN), deltaTag.getString(ResourceReference.RESOURCE_PATH));
            	
                //if (removeAttributes.contains(attributes.getCompoundTagAt(i).getString("Name")))
            	if(removeAttributes.contains(ResourceReference.getResourceString(resource)))
                {
    ...
    public static Entity getTargetEntity(Entity input, double maxReach)
    {
        ...
        List<Entity> list = input.world.getEntitiesInAABBexcluding(input, input.getEntityBoundingBox().expand(look.x * maxReach, look.y * maxReach, look.z * maxReach).expand(area, area, area), new Predicate<Entity>()
        {
            @Override
        ...
        for (int i = 0; i < list.size(); ++i)
        {
            ...
            AxisAlignedBB aabb = entity.getEntityBoundingBox().expand(entity.getCollisionBorderSize(), entity.getCollisionBorderSize(), entity.getCollisionBorderSize());
