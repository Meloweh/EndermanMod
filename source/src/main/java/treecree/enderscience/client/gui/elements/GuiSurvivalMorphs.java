...
public class GuiSurvivalMorphs extends Gui
{
    ...
    public void setupMorphs(IMorphing morphing)
    {
        ...
        {
            MorphType list = separated.get(ResourceReference.getResourceString(morph.resource));

            if (list == null)
            {
                list = new MorphType();
                separated.put(ResourceReference.getResourceString(morph.resource), list);
            }
        ...
        Collections.sort(this.morphs, new Comparator<MorphType>()
        {
            ...
            {
                return ResourceReference.getResourceString(a.morphs.get(0).morph.resource)
                		.compareTo(ResourceReference.getResourceString(b.morphs.get(0).morph.resource));
            }
        });
    ...
    {
        boolean renderDemorph = EnderScience.proxy.config.show_demorph;
        ...
        for (int i = 0, c = this.morphs.size(); i <= c; i++)
        {
            ...
            String name = EnderScience.proxy.config.hide_username ? "Demorph" : player.getName();

            if (i != 0)
            {
                name = ResourceReference.getResourceString(MorphManager.INSTANCE.morphDisplayResourceFromMorph(morph));
            }
    ...
    public void renderPlayer(EntityPlayer player, int x, int y, int scale)
    {
        ...
        Render render = this.mc.getRenderManager().getEntityRenderObject(entity);
