...
import treecree.enderscience.api.models.IHandProvider;
...
public class EntityMorph extends AbstractMorph {
	...
	public void renderOnScreen(EntityPlayer player, int x, int y, float scale, float alpha) {
		...
		// if (this.resource.equals("Ghast"))
		if (ResourceReference.getResourceString(resource).equals("minecraft:ghast")) {
			scale = 5F;
		} else if (ResourceReference.getResourceString(resource).equals("minecraft:gurardian") && entity.height > 1.8) {
	...
	public void update(EntityLivingBase target, IMorphing cap) {
		...
		/*
		 * Now goes the code responsible for achieving somewhat riding support. This is
		 * ridiculous...
		 */
	...
	public void updateSize(EntityLivingBase target, float width, float height) {
		 ...
		 * 
		 * The problem is that Minecraft changes to correct size of baby animals on the
		 * client, but on the server it doesn't change anything thus I have to rely on
		 * proivded NBT data for figuring out if an animal entity is being a baby
		 */
	...
	public void setupEntity(World world) {
		/*
		 * if(resource.getResourceDomain().isEmpty() ||
		 * resource.getResourcePath().isEmpty()) { return; //resource = new
		 * ResourceLocation(NameReference.DOMAIN_MINECRAFT,
		 * NameReference.ENTITY_PLAYER); }
		 */

		final Entity entity = EntityList.createEntityByIDFromName(resource, world);

		EntityLivingBase created = (EntityLivingBase) entity;
	 ...
	 * Setup renderer
	 * 
	 * This method is responsible for setting up any client side stuff like the
	 * renderer, texture of the entity and the "hands"
	 */
	@SideOnly(Side.CLIENT)
		...
		for (Method method : clazz.getDeclaredMethods()) {
			...
			if (hasEntityArg && returnsRL) {
				try {
					...
					EnderScience.log("Failed to get texture of a morph '" + ResourceReference.getResourceString(resource)
							+ "'!");
					e.printStackTrace();
				}
	 ...
	 * 
	 * This guy is responsible for finding {@link ModelRenderer} in renderer's main
	 * model.
	 * 
		...
		if (model instanceof IHandProvider) {
			...
			Collections.sort(left, new Comparator<ModelRenderer>() {
				...
				public int compare(ModelRenderer a, ModelRenderer b) {
					return (int) (a.rotationPointX - b.rotationPointX < 0
							? Math.floor(a.rotationPointX - b.rotationPointX)
							: Math.ceil(a.rotationPointX - b.rotationPointX));
				}
			});
			...
			Collections.sort(right, new Comparator<ModelRenderer>() {
				@Override
				public int compare(ModelRenderer a, ModelRenderer b) {
					return (int) (b.rotationPointX - a.rotationPointX < 0
							? Math.floor(b.rotationPointX - a.rotationPointX)
							: Math.ceil(b.rotationPointX - a.rotationPointX));
				}
			});
	 ...
	 * 
	 * This method is going to be used for saving entity state to morph capability.
	 */
